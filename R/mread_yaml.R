...
#' @importFrom mrgsolve mread_cache
#' @importFrom tools file_path_sans_ext
#' @importFrom purrr imap map map_dbl map_chr imap_dfr
...
mread_yaml <- function(file, project = '.', ...) {
  cpp <- yaml_to_cfile(file,project)
  mread_cache(cpp, project = project, ...)
}


yaml_to_cfile <- function(file,project,ext=".model") {
...
parse_reactions <- function(eq,width = NULL,width_plus = 1,j_prefix = "J",re=NULL) {
  if(is.null(re)) re <- "(.*?)(<*-+>)(.*)"
  species0 <- map(eq, "species")
  species0 <- gsub(" ", "", species0, fixed = TRUE)
  species <- str_match(species0,re)
  stopifnot(ncol(species) ==4)
  species <- as.data.frame(species,stringsAsFactors = FALSE)
  if(any(is.na(species[,2]))) {
    bad_species_i <- which(is.na(species[,2]))
  ...
  formula <- map_chr(eq, "form")
  label <- map_chr(eq, "label", .default = "")
